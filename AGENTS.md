...
> `/sync-protocol`. Shape these to the stack: a web app, an ML pipeline, a game,
...
or run `/sync-protocol`. The rows below are examples for a typed web stack, shape
...
output diff against a baseline, a sanitizer run). Generated by `/sync-protocol`
from the stack.

### Stack-specific rules

Conventions unique to this stack, added by `/sync-protocol` or by you.
