...
This is a pragmatic experiment in managing AI-assisted software projects. It attempts to solve the friction I keep hitting when starting fresh repositories. 

## The Problem

We usually jump straight into scaffolding. We ask an agent to "build a web app" and watch it generate boilerplate. 

Then the thrashing begins. We realize halfway through that we picked the wrong database. We switch libraries. We rewrite core logic. We end up spending more time fighting the agent's assumptions than building the product, all because the requirements were never broken down properly in the first place.
