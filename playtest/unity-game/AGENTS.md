...
Lumen (working title): a cozy 2D, side-on exploration and light-puzzle game built
in Unity (C#). No combat, no fail state. PC via Steam first, Nintendo Switch a
later target. Phase: early (pre-production toward a vertical slice). Locked: Unity
+ C#, 2D hand-painted art with skeletal animation, a hard 60fps target, and no
microtransactions.

### Toolchain

Unity, not a web stack: there is no package-manager install step and no `tsc`, and
C# type errors surface at compile/build time. Commands assume the Unity version
pinned in `ProjectSettings/ProjectVersion.txt`, invoked headless as `unity` below.

| Action          | Command                                                                                 |
| --------------- | --------------------------------------------------------------------------------------- |
| restore assets  | `git lfs pull` (binary art); UPM packages resolve on first Editor open                   |
| format          | `dotnet format` (rules in `.editorconfig`)                                               |
| analyze / lint  | `dotnet format --verify-no-changes`; Roslyn analyzers run during the build              |
| test            | `unity -batchmode -runTests -testPlatform EditMode` and `-testPlatform PlayMode`         |
| run / dev       | open in the Unity Editor and enter Play Mode (interactive)                               |
| build (player)  | `unity -batchmode -quit -buildTarget StandaloneWindows64 -executeMethod BuildScript.Run` |
| package manager | Unity Package Manager (`Packages/manifest.json`); Git LFS for binary assets              |

### Validation gates

In order, a slice is done when:

1. `dotnet format --verify-no-changes` is clean (analyzers included).
2. EditMode and PlayMode tests pass (Unity Test Framework, headless).
3. The headless StandaloneWindows64 player build succeeds.
4. Playtest the touched flow in Play Mode: it behaves, and the frame budget holds
   (60fps target). This gate is observed, not a command, and is required for
   anything player-facing.

### Stack-specific rules

- Never hand-edit `.meta` files; let Unity own them. Never resolve a scene or
  prefab merge conflict blind: configure UnityYAMLMerge (Smart Merge), or redo the
  change by hand. Treat scenes and prefabs as effectively binary.
- Keep gameplay deterministic where feasible (fixed timestep, seeded RNG) so
  playtests reproduce.
- Put an input abstraction in from the start and hold the 60fps budget against
  Switch-class hardware; the Switch port is a later target, not a rewrite.
- Save anywhere. No microtransactions, ever.
- Binary art (hand-painted backgrounds, 2D skeletal rigs) lives under Git LFS.
- Audio is first-class (adaptive music); keep a seam for an FMOD integration later.
